...
int main(void)
{
    ...
    // p_sale = &mysale[0];
    //next line also works to assign address to pointer
    // because an array without [] indicates the address
    // of the first element of said array
    p_sale = mysale;

    //NOTE: EACH ELEMENT BELOW IS INITIALIZED WITH WHATEVER STRINGS USING THE THREE DIFFERENT METHODS
    // AVAILABLE LISTED ON PAGE 263 BOTTOM
    //  	1. STR.MEM
    // 	2. (*PTR).MEM
    // 	3. PTR->MEM
